	...
	///////////////////////////////////////////////////////////////////////////
		...
		while($periodo<=$anio_tmp){
			...
			while($row=mysqli_fetch_array($resp)){
				...
				$entra=0;
				if ($row['monto']>0){
					$totala=number_format($row['monto'],2);
					$entra=1;
					$ahorrop+=$row['monto'];
				}
				else if($row['monto']<0){
					$monto=$row['saldofinal']-$monto;
					$ahorrop=$monto;
					$totalr=number_format($row['monto'],2);
				...
				if($entra==1){
					$data[$i]=array('anio'=>$row['anio'],'quin'=>$row['quin_nombre'],'ret'=>$totalr,'apor'=>$totala);
					$i++;
				}
			}
