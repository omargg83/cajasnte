<?php
	...
	$id=$_SESSION['idfolio'];
		...
		include '../librerias/pdf/src/Cezpdf.php'; // Or use 'vendor/autoload.php' when installed through composer
		error_reporting(E_ALL);
		// Initialize a ROS PDF class object using DIN-A4, with background color gray
		$pdf = new Cezpdf('letter','portrait');
		$pdf->tempPath = '../librerias/pdf/src/fonts';
		// Set pdf Bleedbox
		...
		while($saldofinal=mysqli_fetch_array($resp)){
			$data[$i]=array('anio'=>$saldofinal['anio'],'quin'=>$saldofinal['quin_nombre'],'apor'=>number_format($saldofinal['monto'],2));
			$i++;
		}
