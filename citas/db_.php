...
class Escritorio extends Sagyc{
	...
	public function citas(){
					...
					$t="$sql<div class='card'>";
						...
						$t.= "<input class='form-control' type='hidden' id='tipo' name='tipo' value='$tipo' readonly>";
							$t.="<div class='row'>";
								$t.= "<div class='col-3'>";
										$t.= "<label># Cita</label>";
										$t.= "<input class='form-control' type='text' id='cita' name='cita' value='$x' readonly>";
								$t.= "</div>";
								$t.= "<div class='col-3'>";
										$t.= "<label>Fecha</label>";
		...
		public function citas_afiliados(){
			try{
				self::set_names();
				$sql="select * from citas where idfolio=:idfolio order by fecha desc";
				$sth = $this->dbh->prepare($sql);
